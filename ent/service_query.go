...
type ServiceQuery struct {
	...
	withVersions *ServiceVersionQuery
...
func (sq *ServiceQuery) QueryVersions() *ServiceVersionQuery {
	query := (&ServiceVersionClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withVersions: sq.withVersions.Clone(),
...
func (sq *ServiceQuery) WithVersions(opts ...func(*ServiceVersionQuery)) *ServiceQuery {
	query := (&ServiceVersionClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withVersions = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := sq.withVersions; query != nil {
		if err := sq.loadVersions(ctx, query, nodes,
			func(n *Service) { n.Edges.Versions = []*ServiceVersion{} },
			func(n *Service, e *ServiceVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServiceQuery) loadVersions(ctx context.Context, query *ServiceVersionQuery, nodes []*Service, init func(*Service), assign func(*Service, *ServiceVersion)) error {
	...
	query.Where(predicate.ServiceVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(service.VersionsColumn, fks...))
	}))
